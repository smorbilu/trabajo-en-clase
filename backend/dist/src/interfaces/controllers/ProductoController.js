...
class ProductoController {
    static async getAll(_req, res) {
        try {
            const productos = await ProductoModel_1.ProductoModel.findAll({
                include: [
                    {
                        model: CategoriaModel_1.CategoriaModel,
                        as: "categoria",
                    },
                ],
                order: [["id", "ASC"]],
    ...
    static async create(req, res) {
        try {
            ...
            const categoria = await CategoriaModel_1.CategoriaModel.findByPk(Number(categoria_id));
            if (!categoria) {
                res.status(400).json({ mensaje: "La categoría seleccionada no existe" });
                return;
            }
            const nuevoProducto = await ProductoModel_1.ProductoModel.create({
    ...
    static async update(req, res) {
        try {
            const { id } = req.params;
            const { nombre, descripcion, precio, stock, imagen, categoria_id } = req.body;
            const producto = await ProductoModel_1.ProductoModel.findByPk(Number(id));
            if (!producto) {
